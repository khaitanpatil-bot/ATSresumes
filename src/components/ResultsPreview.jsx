...
export default function ResultsPreview() {
  return (
    <section className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="rounded-[2rem] bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        ...
        <div className="mt-8 rounded-[2rem] bg-slate-950/90 p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="rounded-full bg-slate-800 px-4 py-2 text-sm text-emerald-300">Good Match</div>
          </div>
      ...
      <div className="rounded-[2rem] bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        <h3 className="text-xl font-semibold text-white">Issues Found (5)</h3>
        <p className="mt-3 text-slate-400">Improve these areas to boost your ATS score.</p>
        <ul className="mt-6 space-y-4 text-slate-200">
          {issues.map((issue, index) => (
            <li key={issue} className="flex items-start gap-3 rounded-3xl bg-slate-950/70 p-4">
              <span className="mt-1 inline-flex h-8 w-8 items-center justify-center rounded-full bg-amber-400/15 text-amber-300">
                {index + 1}
              </span>
              <span>{issue}</span>
            </li>
